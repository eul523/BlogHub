...
export default function Post({ title, body, likes_count, comments_count, slug, author, dateWritten, images, tags, liked:likedN, isOwner, isFavourite }) {
    ...
    return (
        <>
            <div className="w-[90%] max-w-[800px] border-b border-b-gray-500 flex justify-between my-2">
                <div className="flex flex-col items-center justify-center mt-0">
                    <div className="flex flex-2 flex-col max-w-[800px]   w-full mt-0">
                        ...
                        <div className="sm:hidden flex overflow-x-scroll gap-4 my-4">
                            {images.length>0 &&
                               images.map(i=>(
                                <img key={i} src={import.meta.env.VITE_BASE_URL + i} className="h-[110px] aspect-auto"/>
                               ))
                            }
                ...
                {images?.length > 0 &&
                        <img key={images[0]} className="h-[100px] max-w-[50%]  aspect-auto border mt-[5%] hidden sm:block" src={import.meta.env.VITE_BASE_URL + images[0]} />
                    }
